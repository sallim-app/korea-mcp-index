...
`why`는 채점자의 총평을 **자르지 않고** 담는다. 자르는 일은 표를 그리는 쪽(render)이
문장 경계에서 한다 — 여기서 잘라 두면 원자료(`ranking.json`)를 읽는 사람도 잘린 것만 본다.
사실오류는 건수를 따로 싣고 인용 전문은 `grades/`에 있다.
...
def main() -> int:
    ...
    for f in sorted(pathlib.Path("grades").glob("*.json")):
        ...
        for name in d.get("권장순위") or []:
            ...
            repro = s.get("재현성")
            # `why`는 **총평만** 담는다(2026-08-19 눈으로 읽고 고침). 종전엔 앞에
            # `사실오류 N건 — '<인용 60자>' · `를 붙였는데, 60자가 인용을 문장 중간에서
            # 자르고("이 데이터셋은  · ", "행정안전부( · ") 뒤이어 총평까지 잘려 두 번
            # 깨졌다. 사실오류는 이미 표에 **건수 열**로 나가고 인용 전문은 grades/에
            # 있으니, 순위 줄에서 잘린 인용을 또 보여 줄 이유가 없다.
            why = s.get("총평", "")
            top.append({"name": name, "rank": rank, "why": why,
        ...
        items[f"graded:{cat}"] = {
            ...
            "note": d.get("분야_총평", ""), "candidates": len(d["servers"]),
            "by": "opus(실제 질문·답변 채점)", "순위_근거": d.get("순위_근거", "")}
