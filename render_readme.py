...
def main() -> int:
    ...
    # 4층(2026-09-15, T-2026W38-309): 위 세 원천은 매 회차 세상을 새로 긁으므로 별 0~2개짜리
    # 서버는 GitHub 검색 상위 100건 밖으로 밀리는 것만으로 표에서 사라진다(실측 3건).
    A("         + 게시 이력 이어받기 — 한 번 실은 서버가 안 잡히면 되살려 다시 판정한다")
